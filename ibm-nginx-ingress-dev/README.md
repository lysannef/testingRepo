...
```console
$ helm install stable/ibm-nginx-ingress-dev
```
...
This chart bootstraps an ibm-nginx-ingress-dev deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.

## Chart Details

This chart bootstraps an ibm-nginx-ingress-dev
...
```console
$ helm install --name my-release stable/ibm-nginx-ingress-dev
```

The command deploys ibm-nginx-ingress-dev on the Kubernetes cluster in the default configuration. The `configuration` section lists the parameters that can be configured during installation.
...
The following table lists the configurable parameters of the ibm-nginx-ingress-dev chart and their default values.
...
```console
$ helm install stable/ibm-nginx-ingress-dev --name my-release \
...
```console
$ helm install stable/ibm-nginx-ingress-dev --name my-release -f values.yaml
```
...
```console
$ helm install stable/ibm-nginx-ingress-dev --set controller.extraArgs.v=2
```
